...
const { processMergeProposal } = require('../registry-server/src/workers/merge-worker');
...
// Create bare repo
execSync(`git init --bare "${repoName}"`, { cwd: TEST_ROOT });

// Clone to create content
const clonePath = path.join(TEST_ROOT, 'client-clone');
execSync(`git clone "${repoPath}" "${clonePath}"`);

// Create Main (Base)
const baseManifest = { permissions: { network: { outbound: ['google.com'] } }, version: "1.0.0" };
fs.writeFileSync(path.join(clonePath, 'manifest.json'), JSON.stringify(baseManifest));
execSync(`git add . && git commit -m "Initial commit"`, { cwd: clonePath });
execSync(`git push origin master`, { cwd: clonePath }); // using master as default often

// Create Feature (Head)
execSync(`git checkout -b feature/add-openai`, { cwd: clonePath });
const headManifest = { permissions: { network: { outbound: ['google.com', 'openai.com'] } }, version: "1.1.0" };
fs.writeFileSync(path.join(clonePath, 'manifest.json'), JSON.stringify(headManifest));
execSync(`git add . && git commit -m "Add OpenAI"`, { cwd: clonePath });
execSync(`git push origin feature/add-openai`, { cwd: clonePath });
...
(async () => {
    try {
        ...
        if (result.status === 'MERGED') {
            ...
            // Verify commit graph
            execSync(`git pull origin master`, { cwd: clonePath });
            const log = execSync(`git log --oneline -n 1`, { cwd: clonePath }).toString();
