...
function generateGitlobsterJson(pkg, version, manifestData) {
  ...
  if (manifest.keywords) {
    ...
    try {
      gitlobster.keywords = typeof pkg.tags === 'string' ? JSON.parse(pkg.tags) : pkg.tags;
    } catch (e) { /* ignore */ }
  }
...
async function extractTarball(tarballPath, destDir) {
  ...
  try {
    execSync(`tar -xzf "${tarballPath}" -C "${destDir}"`, { encoding: 'utf-8' });
...
async function initGitRepo(repoPath, packageName, version) {
  const workDir = path.join(TEMP_DIR, `work-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);

  try {
    // Create working directory and init git
    fs.mkdirSync(workDir, { recursive: true });
    execSync('git init', { cwd: workDir, encoding: 'utf-8' });

    // Configure git identity
    execSync('git config user.name "GitLobster Migration"', { cwd: workDir, encoding: 'utf-8' });
    execSync('git config user.email "migration@gitlobster.network"', { cwd: workDir, encoding: 'utf-8' });
    ...
    if (status.trim()) {
      ...
      execSync(`git commit -m "Initial commit: ${packageName}@${version}"`, { cwd: workDir, encoding: 'utf-8' });

      // Get commit hash
      const hash = execSync('git rev-parse HEAD', { cwd: workDir, encoding: 'utf-8' }).trim();

      // Create bare repo
      if (fs.existsSync(repoPath)) {
        fs.rmSync(repoPath, { recursive: true });
      }
      execSync(`git clone --bare "${workDir}" "${repoPath}"`, { encoding: 'utf-8' });
 ...
 */
async function addVersionCommit(repoPath, packageName, version, tarballPath, pkg, versionData) {
  const workDir = path.join(TEMP_DIR, `work-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);

  try {
    // Clone the bare repo to work dir
    execSync(`git clone "${repoPath}" "${workDir}"`, { encoding: 'utf-8' });
    ...
    const gitlobsterJson = generateGitlobsterJson(pkg, { version }, manifestData);
    ...
    const updatedReadme = ensureReadmeFrontmatter(
      readmeContent,
      pkg.name,
      pkg.description || (manifestData ? JSON.parse(manifestData).description : null)
    );
    ...
    if (status.trim()) {
      // Create commit with version info
      execSync(`git commit -m "Version ${version} of ${packageName}"`, { cwd: workDir, encoding: 'utf-8' });

      // Push to bare repo
      execSync('git push origin master', { cwd: workDir, encoding: 'utf-8' });
      ...
      for (let i = 0; i < versions.length; i++) {
        ...
        try {
          if (i === 0) {
            ...
            commitHash = await addVersionCommit(repoPath, pkg.name, v.version, tarballPath, pkg, v);
          }
