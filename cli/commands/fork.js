...
export async function forkCommand(parentPackage, forkedPackage, options) {
  ...
  try {
    ...
    if (!resolvedForkedPackage) {
      ...
      try {
        const { readFile } = await import('fs/promises');
        const localConfig = JSON.parse(await readFile(resolve(process.cwd(), 'gitlobster.json'), 'utf-8'));
        if (localConfig.name) {
    ...
    try {
      ...
      spinner.succeed(`Found ${chalk.cyan(parentPackage)} @ ${chalk.cyan(latestVersion)}`);
    } catch (metaError) {
      spinner.warn(`Registry metadata unavailable (${metaError.message}). Using version placeholder.`);
    }
    ...
    if (!response.ok) {
      ...
      const errMsg = errBody.error || errBody.message || `HTTP ${response.status} ${response.statusText}`;
      throw new Error(`Fork API call failed: ${errMsg}`);
    }

    forkResult = await response.json();
    spinner.succeed(chalk.green(`Forked ${chalk.bold(parentPackage)} → ${chalk.bold(resolvedForkedPackage)}`));

    // ─── Display fork result ──────────────────────────────────────────────────

    const gitUrl = forkResult.git_url || `${registryUrl}/git/${toGitRepoName(resolvedForkedPackage)}.git`;
    ...
    console.log(`  ${chalk.dim('🍴 Forked From:')}  ${chalk.cyan(parentPackage)} ${chalk.dim(`(v${forkPointVersion})`)}`);
    console.log(`  ${chalk.dim('📦 Fork UUID:')}    ${chalk.yellow(forkUuid)} ${chalk.dim('(permanent lineage anchor)')}`);
    console.log(`  ${chalk.dim('🔗 Git URL:')}      ${chalk.cyan(gitUrl)}`);

    if (forkResult.forked_at) {
      console.log(`  ${chalk.dim('🕐 Forked At:')}    ${chalk.gray(forkResult.forked_at)}`);
    }
