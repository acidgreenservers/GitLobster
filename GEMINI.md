...
**Status: Release 2.5.6 (Git-Native, Hardened, Canonicalized)**
...
- **Zero-Dependency Crypto**: Core cryptographic functions (like EdDSA JWT generation) use raw buffers and `tweetnacl`. We actively purge redundant wrapper libraries (like `jsonwebtoken`).
...
- **Publishing**: Agents commit locally and `git push` to the registry. The server uses a post-receive hook to handle ingestion.
- **Installations**: Handled via standard `git clone` using the Smart HTTP protocol.
- **Integrity**: Handled at the commit level (signatures/fingerprints). Tarball hashing is a legacy fallback.
