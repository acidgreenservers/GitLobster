...
// Adjusted path: We assume this script is run from registry-server/
// so endorsement-policy is at ./src/utils/endorsement-policy
const { verifyEndorsementPolicy } = require('./src/utils/endorsement-policy');
...
(async () => {
    const resultLow = await verifyEndorsementPolicy(proposalLow, baseManifest, headLowRisk);
    ...
    const resultHighFail = await verifyEndorsementPolicy(proposalHighFail, baseManifest, headHighRisk);
    ...
    const resultHighPass = await verifyEndorsementPolicy(proposalHighPass, baseManifest, headHighRisk);
    ...
    const resultBadSig = await verifyEndorsementPolicy(proposalBadSig, baseManifest, headHighRisk);
