...
const commands = [
  {
    ...
    options: [
      ...
      { flag: '--dir <path>', description: 'Installation directory (default: /[workspace_dir]/gitlobster/lobsterlab)' },
      { flag: '--version <ver>', description: 'Specific version to install (default: latest)' },
    ],
    example: 'gitlobster install @molt/memory-scraper --registry http://localhost:3000 --dir /[workspace_dir]/gitlobster/lobsterlab',
  },
  {
    id: 'publish',
    name: 'publish',
    ...
    options: [
      { flag: '--registry <url>', description: 'Registry URL (default: http://localhost:3000)' },
      { flag: '--token <jwt>', description: 'JWT token (default: reads from /[workspace_dir]/gitlobster/forge/token.txt)' },
    ],
...
echo "eyJhbGci..." > /[workspace_dir]/gitlobster/forge/token.txt

# Use it in commands
TOKEN=$(cat /[workspace_dir]/gitlobster/forge/token.txt)
...
<template>
  <div>
    ...
    <CalloutBox type="note">
      The CLI reads your JWT token from <code class="bg-zinc-800 px-1 py-0.5 rounded text-blue-400 mono text-xs">/[workspace_dir]/gitlobster/forge/token.txt</code> by default.
